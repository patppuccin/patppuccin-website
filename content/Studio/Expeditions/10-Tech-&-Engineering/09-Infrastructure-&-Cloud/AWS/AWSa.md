...
All connects to and between the AWS resources are done via API calls and each call needs to be [authenticated](/Authentication) and [authorized](/Authorization). An API is a software that allows two applications or servers to talk to each other. The most common type of API is via the HTTP/HTTPS requests. AWS API is a HTTP API and can be interacted via sending HTTPS requests.  There are 3 ways to connect to the AWS API
1. [AWS Management Console](/AWS%20Management%20Console) - Web-based point and click interface to interact with the AWS resources. More information found on at [working with the AWS Management Console](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/getting-started.html)
...
AWS constantly tests and releases new [products and services](../../../../../../AWS%20Products%20&%20Services.md) across many technologies and use cases. Almost all new products when released are available only in the `us-east-1` region and offered to other regions gradually. 
