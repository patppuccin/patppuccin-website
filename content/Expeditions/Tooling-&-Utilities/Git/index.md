...
> There is a modern alternative to `git` written in the [Rust programming language](../../Programming/Languages/Rust/index.md) called [jiujitsu](https://www.jj-vcs.dev/) or `jj`. It is designed to be a drop in replacement to `git`, but with a modern feature-set and a more intuitive user experience.
...
> 
> That being said, in Windows [PowerShell](../../Programming/Languages/PowerShell/index.md) is the preferred Shell of choice. All of these commands work on PowerShell as well. Just being mindful on the slash type (`/`or`\`) is required nonetheless.
